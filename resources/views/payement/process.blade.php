<x-guest-layout>
    <div class="flex flex-col md:flex-row gap-4 h-full">
            ...
            <ol class="flex w-[100%] my-3 px-3">
                <li class="relative mb-6 sm:mb-0 w-[100%]">
                    ...
                    <div class="mt-3 sm:pr-8">
                        <h3 class="text-lg font-semibold dark:text-gray-400 text-gray-900 hidden md:block ">{{ __('Depart') }}
                        </h3>
                ...
                @foreach ($booking->prolongations as $prolongation)
                    <li class="relative mb-6 sm:mb-0 w-[100%]">
                        ...
                        <div class="mt-3 sm:pr-8">
                            <h3 class="text-lg font-semibold dark:text-white text-gray-900 hidden md:block ">prolongation</h3>
                            <time class="block mb-2 text-sm font-normal leading-none text-gray-400">
                ...
                <li class="relative mb-6 sm:mb-0 w-[100%]">
                    ...
                    <div class="mt-3 sm:pr-8 text-right">
                        <h3 class="text-lg font-semibold dark:text-white text-gray-900 hidden md:block ">{{ __('Return') }}</h3>
                        <time class="block mb-2 text-sm font-normal leading-none text-gray-400">
        ...
        <div class="flex w-full p-4 dark:text-gray-400 text-gray-900">

            @if ($booking->financial_status === 'non payé')
                @if ($booking->agencie->online)
                    <form action="{{ route('Transaction.process', $booking->id) }}"><input type="submit"
                            value="paypal">
                    </form>
                @endif
                @if ($booking->reservation_status === 'non confirmée')
                    <form action="{{ route('bookings.index') }}"><input type="submit" value="at agency"></form>
                    <form action="{{ route('bookings.cancel', $booking->id) }}"><input type="submit" value="cancel">
                    </form>
                @endif
            @else
                <div class="w-full flex justify-center flex-col p-8 bg-green-200 rounded-md shadow-md">
                    <h2 class="text-2xl font-semibold mb-6 text-center">Payment Successful</h2>
                    <p class="text-green-700 text-center">Thank you for your payment!</p>
                </div>
            @endif
        </div>
    </div>
